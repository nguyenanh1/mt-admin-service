...
public interface MovieRepo extends CrudRepository<MovieEntity,Integer> {
    ...
    @Query(value = "SELECT * FROM movie_cinema.movie where movie_cinema.movie.part_time < :part_date limit :start,:limit ", nativeQuery = true)
    ...
    List<MovieEntity> getListMovieCommingSoon(@Param("part_date")int part_date,
                                            @Param("start")int start,
                                            @Param("limit")int limit);
    @Query(value = "SELECT  * from movie_cinema.movie order by movie_cinema.movie.view desc limit 5" ,nativeQuery = true)
